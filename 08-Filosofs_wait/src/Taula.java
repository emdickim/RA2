public class Taula {
    ...
    public Taula(int numeroFilosofs) {
        ...
        for (int i = 0; i < numeroFilosofs; i++) {
            filosofs[i] = new Filosof("fil" + i, i);
            forquilles[i] = new Forquilla(i);
        }

        for (int i = 0; i < filosofs.length; i++) {
            filosofs[i].forquillaEsquerra = forquilles[i];
            filosofs[i].forquillaDreta = forquilles[(i + 1) % numeroFilosofs];
        }
    ...
    public static void showTaula() {
        for (int i = 0; i < filosofs.length; i++) {
            System.out.printf("Comensal :%s esq:%d dret:%d%n" , filosofs[i].getName(), filosofs[i].forquillaEsquerra.getNumero(), filosofs[i].forquillaDreta.getNumero());
        }
    }
